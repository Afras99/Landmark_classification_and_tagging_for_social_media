...
def get_model_transfer_learning(model_name="resnet18", n_classes=50):
    ...
    custom_head = nn.Sequential(
        nn.Linear(num_ftrs, 256),  # First linear layer with 256 output units
        nn.ReLU(),                 # Activation function (e.g., ReLU)
        nn.Linear(256, 128),       # Second linear layer with 128 output units
        nn.ReLU(),                 # Activation function (e.g., ReLU)
        nn.Linear(128, n_classes)  # Third linear layer with n_classes output units
    )
    # Replace the fully connected (fc) layer with the custom head
    model_transfer.fc = custom_head
